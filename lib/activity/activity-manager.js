...
(() => {
  ...
  class ActivityManager extends PouchDBCrudManager {
    ...
    create({title, projectName, icon='icons:build', createdAt, ongoing=false, elementContent, elementImport, data, notify=false, notifyOptions, url, defconLevel=ActivityApi.DEFCON.DOUBLE_TAKE}={}) {
    ...
    list(query={}, options={}) {
      ...
      const chain = this._chain.then(() => super.list(query, options));
