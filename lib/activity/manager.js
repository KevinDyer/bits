...
(() => {
  ...
  const {PouchDbCrudManager} = require('@lgslabs/bits-pouch-crud');
  const {DEFCON} = require('./constants');

  class ActivityManager extends PouchDbCrudManager {
    constructor() {
      super();
      this._boundDeleteItemsOverActivityLimit = null;
      this._chain = null;
      this._settings = null;
    }

    load({path, messageCenter, settings}) {
      this._boundDeleteItemsOverActivityLimit = this._deleteItemsOverActivityLimit.bind(this);
      this._chain = Promise.resolve();
      this._settings = settings;

      return Promise.resolve()
      .then(() => super.load({path, messageCenter}))
    ...
    create({title, projectName, icon='icons:build', createdAt, ongoing=false, elementContent, elementImport, data, notify=false, notifyOptions, url, scopes=[], defconLevel=DEFCON.DOUBLE_TAKE}={}) {
