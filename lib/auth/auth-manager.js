...
(() => {
  ...
  const {LocalResource, ResourceManager} = require('@lgslabs/bits-auto-discovery');
  ...
  class AuthManager extends EventEmitter {
    ...
    load(messageCenter) {
      const localResource = new LocalResource({topic: 'webserver-express-router-api', value: {
        filepath: path.resolve(__dirname, './auth-router'),
        routePath: '/api/base/auth',
        params: {useBodyParser: true},
        isAuthenticated: false,
      }});

      // TODO: bschmickley - get API resource
      // this._userApi.on('deleted', this._onUserDeleted.bind(this));
      ...
      .then(() => localResource.load(messageCenter));
    }
    ...
    signInWithUsernameAndPassword(username, password) {
      ...
      // Authenticate
      return Promise.resolve()
      .then(() => this._userApi.authenticate(username, password))
      .catch((err) => {
    ...
    validateAccessToken(token) {
      ...
      .then((accessToken) => {
        if (accessToken) {
          return this._userApi.get(accessToken.userId)
          .then((user) => {
            if (user) {
              accessToken.user = user;
              return accessToken;
