...
(() => {
  ...
  const TOPIC = {
    ROOT_ROUTE: 'base-server-root-router',
    STRATEGY_API: 'passport-strategy-api',
  };
  ...
  class BaseServer extends EventEmitter {
    constructor(authManager, keyManager, certManager) {
      ...
      this._routerResourceMgr = new ResourceManager({topic: TOPIC.ROOT_ROUTE});
      this._routerResourceMgr.on('add', this._onRouterAdded.bind(this));

      this._strategyResourceMgr = new ResourceManager({topic: TOPIC.STRATEGY_API});
      this._strategyResourceMgr.on('add', this._onStrategyAdded.bind(this));
      this._strategyResourceMgr.on('remove', this._onStrategyRemoved.bind(this));
    ...
    load(messageCenter) {
      ...
      return Promise.resolve()
      .then(() => this._routerResourceMgr.load(messageCenter))
      .then(() => this._strategyResourceMgr.load(messageCenter))
      .then(() => this.listen());
    }

    unload() {
      return Promise.resolve()
      .then(() => this._strategyResourceMgr.removeAllListeners())
      .then(() => this._strategyResourceMgr.unload());
    }
    ...
    _onRouterAdded(resource) {
      return Promise.resolve()
      .then(() => {
        const {path, root, type} = resource.getValue();

        switch (type) {
          case 'route':
            try {
              const Api = require(path);
              const api = new Api(this._messageCenter);
              this.app.use(api.middleware);
            } catch (err) {
              logger.err(`Error loading ${TOPIC.ROOT_ROUTE} for ${path}`, err);
            }
            break;
          case 'static':
            ...
            break;
          default:
            logger.error(`Unknown ${TOPIC.ROOT_ROUTE} type: ${type}`);
            break;
        }
      });
    }
