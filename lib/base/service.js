...
(() => {
  ...
  // Need to export the Helper API here, otherwise the Helper Service will try to use its own API to publish its API.
  ...
  class BaseService {
    load() {
      ...
      .then(() => {
        const messageCenter = new MessageCenter(executor);
