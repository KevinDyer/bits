...
(() => {
  ...
  */
  ...
  global.helper.add('BaseOmgsApi', path.join(__dirname, 'omgs/omgs-api'));
  ...
  global.helper.add('PagesApi', path.join(__dirname, 'helpers/pages-manager'));
  global.helper.add('PagesManager', path.join(__dirname, 'helpers/pages-manager'));
  global.helper.add('PagesMessenger', path.join(__dirname, 'helpers/pages-messenger'));
  global.helper.add('PagesService', path.join(__dirname, 'helpers/pages-service'));
  ...
  class Base extends EventEmitter {
    ...
    initialize() {
      ...
      const UpgradeManager = require('./upgrade/upgrade-manager');
      ...
      this._upgradeManager = new UpgradeManager();
      this._moduleManager = new ModuleManager({scopeManager: this._scopeManager,
        cryptoManager: this._cryptoManager,
        userManager: this._userManager,
        loggingManager: this._loggingManager,
        galleryItemManager: this._galleryItemManager,
        widgetManager: this._homeManager.getWidgetManager(),
        pageItemManager: this._pageItemManager,
        keyManager: this._keyManager});
      this._omgManager = new OmgsManager(this._cryptoManager, this._upgradeManager);
      ...
      .then(() => this._homeManager.load(this._messageCenter, this._bitsId.getBitsId()))
      ...
      .then(() => this._upgradeManager.load(this._messageCenter))
