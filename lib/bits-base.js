...
(() => {
  ...
  global.utils.add('UtilChildProcess', path.join(__dirname, 'utils/child-process'));
  ...
  class Base extends EventEmitter {
    ...
    initialize() {
      ...
      .then(() => {
        ...
        return this._helperService.load({messageCenter: this._messageCenter});
      })
      .then(() => {
      })
      .then(() => {
    ...
    dispatchModule(mod) {
      ...
      return Promise.resolve()
      .then(() => this._helperService.load({messageCenter: this._messageCenter}))
      .then(() => this._dispatchService.load({messageCenter: this._messageCenter, mod}))
      .catch((err) => {
