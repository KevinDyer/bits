...
(() => {
  ...
  const {Encrypter, Decrypter} = require('@lgslabs/bits-crypto');
  ...
  class CryptoManager {
    ...
    decryptFileWithKeyAndOffset(filepath, outdir, encryptionKeyHash, signatureKeyHash, offset) {
      const encryptionKey = this._keyManager.getKeyByHash(encryptionKeyHash);
      if (!encryptionKey) {
      ...
      const signatureKey = (signatureKeyHash ? this._keyManager.getKeyByHash(signatureKeyHash) : this._keyManager.getBitsSignaturePublicKey());
      if (!signatureKey) {
    ...
    encryptFile(filepath, encryptionKey, signatureKey, outdir) {
      ...
      let headerEnd = 0;
      return Promise.resolve()
      .then(() => UtilFs.exists(filepath))
      .then((exists) => {
        if (!exists) {
          return Promise.reject(new Error('Can not encrypt; filepath does not exist.'));
        }
      })
      .then(() => UtilFs.stat(outdir))
      .then((stats) => {
        if (!stats.isDirectory()) {
          return Promise.reject(new Error('outdir is not a directory.'));
        }
        const extname = path.extname(filepath);
