...
(() => {
  ...
  const ModuleConstants = require('../modules/module-constants');
  ...
  class Dispatcher {
    ...
    die() {
      ...
      .then(() => {
        if (!(ModuleConstants.isOneshot(this._module))) {
    ...
    _dispatchModule() {
      ...
      .catch((err) => {
        ...
        process.removeListener('uncaughtException', processSeriousError);
        process.removeListener('unhandledRejection', processSeriousError);
        return this._moduleApi.loadComplete({err: {message: err.message, stack: err.stack}})
    ...
    _callModuleIndexJsLoad(mod) {
      if (mod.shotFired) {
        logger.info('Oneshot module, load already call', mod.name);
