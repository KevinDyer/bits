...
(() => {
  ...
  class Dispatcher {
    ...
    _dispatchModule() {
      ...
      .then((result) => {
        return this._moduleApi.loadComplete({result: 'success'});
      })
      .then(() => {
      ...
      .catch((err) => {
        ...
        if (err) {
          ...
          error = {message: 'Unknown Error Occurred', stack: 'No Stack'};
        }
        process.removeListener('uncaughtException', processSeriousError);
        process.removeListener('unhandledRejection', processSeriousError);
        return this._moduleApi.loadComplete({err: {message: error.message, stack: error.stack}})
