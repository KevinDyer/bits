...
(() => {
  ...
  class HelperMessenger extends Messenger {
    _addRequestListeners(options) {
      ...
      .then(() => Promise.all([
        this.addRequestListener(REQUEST.LIST, {scopes: this._scopes}, this._list.bind(this)),
        this.addRequestListener(REQUEST.ADD, {scopes: this._scopes}, this._add.bind(this)),
        this.addRequestListener(REQUEST.REMOVE, {scopes: this._scopes}, this._remove.bind(this)),
      ]));
    }
    ...
    _added(helper) {
      this.sendEvent(EVENT.ADDED, {scopes: this._scopes}, helper);
    }

    _removed(helper) {
      this.sendEvent(EVENT.REMOVED, {scopes: this._scopes}, helper);
    }
  }
