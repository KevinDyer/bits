  ...
  class BaseServer {
    ...
    use(path, middleware, {isAuthenticated=true}={}) {
      ...
      .then(() => {
        ...
        if (this._serving) {
          return Promise.resolve()
          .then(() => this.addProxyPath(path, {isAuthenticated: isAuthenticated}))
          .then((id) => {
            data.id = id;
          });
        }
    ...
    removeMiddleware(path, middleware) {
      ...
      .then(() => {
        ...
        if (data) {
          ...
          if (this._serving) {
            return Promise.resolve()
            .then(() => this.removeProxyPath(data.id))
            .then(() => {
              data.id = null;
            });
          }
    ...
    /* Used if you want to setup a proxy but you dont have middleware to add ie. websocket */
    addProxyPath(path, {isAuthenticated=true} = {}) {
      return Promise.resolve()
      .then(() => this._proxyApi.add({
        workerId: this._workerId,
        path: path,
    ...
    removeProxyPath(id) {
      return Promise.resolve()
      .then(() => this._proxyApi.remove({id: id}));
    }
