(() => {
  ...
  // const BaseBaseServer = require('../base-server');
  ...
  class CrudRouter {
    ...
    __waitForData(req) {
      return new Promise((resolve, reject) => {
        if (this._baseServer instanceof EventEmitter) resolve(req); // No proxying from the OG BaseServer, so just return the request

        req.on('error', reject);
        req.on('data', (buffer) => resolve(JSON.parse(buffer.toString())));
      });
    }
  }
