  ...
  class Daemon extends EventEmitter {
    ...
    start() {
      if (this._childProcess === null) {
        ...
        return Promise.reject(new Error(`Already Running Daemon ${command} ${parameters.join(' ')}`));
      }
    }
    ...
    shutdown() {
      ...
      if (this._childProcess) {
        ...
        return new Promise((resolve, reject) => {
          ...
          onError = (err) => {
            this._childProcess.stdin.removeListener('error', onError);
          ...
          this._childProcess.stdin.on('error', onError);
    ...
    _addListeners() {
      ...
      this._childProcess.on('exit', this._boundOnExit);
      this._childProcess.stdin.on('error', this._boundOnError);
    ...
    _removeListeners() {
      ...
      this._childProcess.removeListener('exit', this._boundOnExit);
      this._childProcess.stdin.removeListener('error', this._boundOnError);
