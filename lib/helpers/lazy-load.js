...
(() => {
  ...
  const BaseModuleApi = require('./../modules/api');
  ...
  class LazyLoad {
    constructor() {
      ...
      this._onLoaded = null;
      this._onUnloaded = null;
      this._isLoaded = null;
      this._messageCenter = null;
      this._moduleApi = null;
      this._boundOnList = null;
    }

    load({messageCenter, moduleName = null, onLoaded = null, onUnloaded = null} = {}) {
      if ('string' !== typeof moduleName) return Promise.reject(new TypeError('moduleName must be a string'));
      if ('function' !== typeof onLoaded) return Promise.reject(new TypeError('onLoaded must be a function'));
      if ('function' !== typeof onUnloaded) return Promise.reject(new TypeError('onUnloaded must be a function'));
