...
(() => {
  ...
  class PagesManager extends EventEmitter {
    ...
    _onResourceAdded(res) {
      Promise.resolve()
      .then(() => this.add(Object.assign({}, res.getValue(), {uuid: res.getUuid()})))
    ...
    validate(page) {
      ...
      .then((page) => {
        ...
        if (!page.hasOwnProperty('priority') || PRIORITY.MIN > page.priority || PRIORITY.MAX < page.priority) page.priority = PRIORITY.MIN;
    ...
    /**
     * Add a page to the PagesManager list
     * @param {Object} page the page to add to the list
     * @return {Promise} A promise to add
     */
    add(page) {
      return Promise.resolve()
      .then(() => this.validate(page))
      .then((page) => {
        if (!this._pages[page.name]) this._pages[page.name] = [];

        const element = this._pages[page.name].length ? this._pages[page.name][0].element : null;

        this._pages[page.name].push(page);
        this._pages[page.name].sort((a, b) => b.priority - a.priority);
        this.emit('page-added', page);

        if (this._pages[page.name][0].element !== element) this._computePageList();
      });
    }
    ...
    load(messageCenter, pages=[]) {
      return Promise.resolve()
      .then(() => Promise.all(pages.map((page) => this.validate(page))))
      .then((validatedPages) => {
