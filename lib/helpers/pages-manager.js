...
(() => {
  ...
  class PagesManager extends EventEmitter {
    ...
    validate(page) {
      ...
      .then((page) => {
        if (page.order < 0) {
          return Promise.reject(new Error('list order must be positive number'));
        }
    ...
    __validate(page) {
      ...
      .then(() => {
        ...
        if (!page.displayName) return Promise.reject(new Error('displayName must be non-empty string'));
