...
(() => {
  ...
  class KeyManager {
    ...
    addKeyFromFilepath(filepath, {copy=false}={}) {
      ...
      .then((stats) => {
        if (stats.isSymbolicLink()) {
          return Promise.reject(new Error('keys/symlink-key'));
        }
      })
      .then(() => Key.fromFile(filepath))
      .then((key) => {
    ...
    getDevicePrivateKey() {
      ...
      .then((privateKeys) => {
        const matches = privateKeys.filter((privateKey) => {
          const keyName = path.basename(privateKey.path, Key.EXT_PRIVATE);
