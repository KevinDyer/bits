...
(() => {
  ...
  class LoggingManager extends CrudManager {
    ...
    generateCrashDump({encrypt = true} = {}) {
      if (!this._cryptoApi) {
        return Promise.reject(new Error('unable to generate crash dump: crypto API unavailable'));
      }
      if (this._waiting) {
        return Promise.resolve();
      }
    ...
    _packageLogDirectories({encrypt, fileroot}) {
      ...
      .then(() => {
        ...
        .then(() => {
          if (encrypt) {
            return Promise.resolve()
            .then(() => this._cryptoApi.encryptFileWithAvailableKeys({filepath: tarFilepath, outdir: os.tmpdir()}))
            .then((result) => {
