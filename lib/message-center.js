  ...
  class MessageCenter extends EventEmitter {
    constructor(cluster, proc) {
      ...
      if (this.cluster.isMaster) {
        ...
        if (proc.env && proc.env.mod) {
          try {
            const {name} = JSON.parse(proc.env.mod);
            this._origin = name;
          } catch (err) {
            logger.error('Error parsing module info from env', err);
          }
        }
    ...
    _handleAddEvent(worker, msg) {
      ...
      // If the worker has never requested the event create it
