...
(() => {
  ...
  function getExecutor(execType = 'default') {
    ...
    if (execType === defaultType) return executors.default;
    const executor = __getExecutor(execType);

    // set up proxy
    executor.on('created', (...args) => executors.default.emit('created', ...args));
    executor.on('message', (...args) => executors.default.emit('message', ...args));
    executors.default.on('send', (...args) => executor.send(...args));
  ...
  function getAllExecutors() {
    return Object.values(executors).filter((executor) => null !== executor);
  }

  const defaultType = __getDefaultType();
  const executors = {
    default: __getExecutor(defaultType),
    process: null,
    thread: null,
  };
