...
(() => {
  ...
  const {getExecutor, getAllExecutors} = require('../module-execution/executor');
  ...
  class ModuleExecutorService extends EventEmitter {
    ...
    unload() {
      return Promise.resolve()
      .then(() => getAllExecutors())
      .then((executors) => executors.map((executor) => executor.removeListener('exit', this._boundOnExecutorExit)))
    ...
    summonTheModule(mod) {
      if (this._modules.hasOwnProperty(mod.id) && !isOneshot(mod)) {
        ...
        const executor = getExecutor(((mod.executor && mod.executor.type) ? mod.executor.type : 'default'));
        if (!this._executors.has(executor)) {
