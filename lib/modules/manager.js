...
(() => {
  ...
  class ModuleManager extends CrudManager {
    ...
    installModule({file} = {}) {
      ...
      .then((result) => {
        return BitsFs.unlink(file.path)
        .then(() => result);
      });
    }
    ...
    _installModule(modInfo, modPath) {
      ...
      .then(() => {
        return BitsFs.rename(modPath, pathToSave)
        .then(() => this._addModule(pathToSave))
        .then(() => BitsFs.exists(path.resolve(pathToSave, 'support/yarn-cache')));
      })
      .then((localCache) => {
        // Run npm run bits:install
        const packageFile = path.resolve(pathToSave, 'package.json');
        return BitsFs.stat(packageFile)
        .then((result) => BitsFs.readFile(path.resolve(packageFile)))
        .then((fileContents) => {
