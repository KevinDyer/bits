...
(() => {
  ...
  class ModuleManager extends CrudManager {
    ...
    _loadModules() {
      ...
      .then(() => this.list())
      .then((mods) => {
        const graph = this._generateDependencyGraph(Array.from(mods)); // 1 level Deep Copy
    ...
    unloadModule(mod, {uninstall=true} = {}) {
      ...
      return this.list()
      .then((modules) => {
        const graph = this._generateDependencyGraph(Array.from(modules)); // 1 level Deep Copy
    ...
    _loadModuleGrouping(graph) {
      ...
      .then(() => this.list())
      .then((items) => {
        const mods = items.reduce((obj, mod) => {
          if (!!mod.isLoaded && !mod.isBase) obj[mod.id] = mod;
          return obj;
        }, {});
        ...
        return new Promise((resolve, reject) => {
          const dispatchModuleChain = () => {
            ...
            if (modulesToLoad.length === 0 && Object.keys(mods).length === 0) {
              return resolve(Object.values(loadingModules));
            }

            Promise.all(modulesToLoad.map((modNode) => {
              const moduleId = modNode.id;
              mods[moduleId] = Promise.resolve()
              .then(() => this.get(moduleId))
