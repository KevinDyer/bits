...
(() => {
  ...
  const {REQUEST, EVENT, SCOPES} = require('./constants');

  class ModuleMessenger extends CrudMessenger {
    _addRequestListeners(options) {
      return super._addRequestListeners(options)
      .then(() => Promise.all([
        this.addRequestListener(REQUEST.INSTALL, {scopes: null}, this._installModule.bind(this)),
        this.addRequestListener(REQUEST.PROVIDE_DATA_DIRECTORY, null, this._provideDataDir.bind(this)),
        this.addRequestListener(REQUEST.GET_DISPLAY_NAME, {scopes: SCOPES}, this._getDisplayName.bind(this)),
        this.addRequestListener(REQUEST.LOAD, {scopes: SCOPES}, this._loadModules.bind(this)),
        this.addRequestListener(REQUEST.UNLOAD, {scopes: SCOPES}, this._unloadModule.bind(this)),
      ]));
    }
    ...
    _installModule(metadata, request) {
      return this._manager.installModule(request);
    }
