  ...
  class ModuleClusterService extends EventEmitter {
    ...
    _destroyTheModule(mod, signal = 'SIGTERM') {
      ...
      .then(() => {
        ...
        if (!modToKill.hasOwnProperty('process')) logger.warn(`${mod.name} has no process to kill. This is unusual`);
    ...
    _addWorkerListeners(mod) {
      ...
      this._modules[mod.id].listeners = {
        disconnect: this._onWorkerDisconnect.bind(this, mod.id),
        error: this._onWorkerError.bind(this, mod.id),
        exit: this._onWorkerExit.bind(this, mod.id),
        online: this._onWorkerOnline.bind(this, mod.id),
      };
