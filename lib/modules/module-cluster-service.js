  ...
  class ModuleClusterService extends EventEmitter {
    ...
    summonTheModule(mod) {
      if (this._modules.hasOwnProperty(mod.id)) {
        ...
        this._modules[mod.id] = {
          id: mod.id,
          name: mod.name,
          restartPolicy: mod.load ? mod.load.restartPolicy : ModuleConstants.RESTART_POLICY.NEVER,
          process: null
        };
        const env = {};
    ...
    destroyTheModule(mod) {
      if (!this._modules.hasOwnProperty(mod.id)) {
        ...
        return Promise.resolve()
        .then(() => this._destroyTheModule(mod))
        .then(() => {
    ...
    _destroyAllListeners(mod) {
      const process = this._modules[mod.id].worker;
      if (!process) {
      ...
      if (this._modules.hasOwnProperty(mod.id)) {
        ...
        .then(() => {
          if (ModuleConstants.isOneshot(mod)) {
            return this._oneshotCompletedLoad(mod);
          } else {
            return this._registerChildProcessHandlers(this._modules[mod.id], mod);
          }
        });
      } else {
    ...
    moduleUnloading(mod) {
      ...
      .then(() => {
        ...
        this._modules[mod.id].worker.once('exit', this._unloadExitHandlers[mod.id]);
      });
    }
    ...
    _registerLoadListeners(mod) {
      ...
      modProcess.worker.on('exit', this._loadExitHandlers[mod.id]);
    }
    ...
    _unregisterLoadListeners(mod) {
      if (!this._loadExitHandlers.hasOwnProperty(mod.id) || !this._modules.hasOwnProperty(mod.id)) {
        ...
        const modProcess = this._modules[mod.id];
        modProcess.worker.removeListener('exit', this._loadExitHandlers[mod.id]);
      }
      delete this._loadExitHandlers[mod.id];
      return Promise.resolve();
    }
    ...
    _registerChildProcessHandlers(modInfo, mod) {
      ...
      modInfo.worker.on('exit', modInfo.exitHandler);
      return Promise.resolve();
    }

    _unregisterChildProcessHandlers(modInfo, mod) {
      if (modInfo.hasOwnProperty('process')) {
        if (modInfo.exitHandler) modInfo.worker.removeListener('exit', modInfo.exitHandler);
      } else {
