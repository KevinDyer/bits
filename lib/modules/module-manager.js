...
(() => {
  ...
  class ModuleManager extends CrudManager {
    constructor({scopeManager, cryptoManager, userManager, loggingManager, galleryItemManager, widgetManager, pageItemManager, keyManager}={}) {
      super(TAG, {
        readScopes: ['base'],
        writeScopes: ['base'],
      ...
      .then(() => {
        ...
        .then((fileContents) => {
          ...
          if (settings && settings.hasOwnProperty('scripts') && settings.scripts.hasOwnProperty('bits:install')) {
            return UtilChildProcess.createSpawnPromise('npm', ['run', 'bits:install'], {cwd: pathToSave});
          } else {
      ...
      return new Promise((resolve, reject) => {
        const dispatchModuleChain = () => {
          ...
          modulesToLoad.forEach((modNode) => {
            ...
            .catch((err) => {
              logger.error('Error loading module %s with id %d', modNode.name, modNode.id, err.message, err.stack);
    ...
    _addGalleryItem(mod) {
      if (mod.icon && mod.displayName) {
        const galleryItem = {
          ...
          href: `/${mod.name}`,
          scopes: Array.isArray(mod.scopes) ? mod.scopes.map((scope) => scope.name) : []
        };
          ...
          .then(() => {
            ...
            if (dep.version && dep.version !== '') { // ignore check for dev modules with no version
              if (!this._satisfies(dep.version, depVersion)) {
                return Promise.reject(new Error(`${mod.name} dependency ${depName} version does not satisfy requirement ${dep.version}`));
              }
            }
          });
        }, Promise.resolve())
