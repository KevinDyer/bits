...
(() => {
  ...
  const BaseLoggingApi = global.helper.BaseLoggingApi;
  const logger = LoggerFactory.getLogger();

  function noop() {}

  class OmgsManager extends EventEmitter {
    constructor(cryptoManager, upgradeManager) {
      super();
      ...
      this._errorsFilepath = path.resolve(global.paths.data, './upgrade/errors');
      this._upgradeLogsDir = path.resolve(global.paths.data, './upgrade');
      this._currentErrorData = null;
      this._boundOnUpgradeStarting = this._onUpgradeStarting.bind(this);
      this._baseLoggingApi = null;
    ...
    load(baseServer, messageCenter) {
      ...
      .then(() => this._addUpgradeToLogging(messageCenter))
      .then(() => this._loadOmgUpgradeErrorFile(this._errorsFilepath))
      ...
      .then(() => this._upgradeManager.on('upgrade-starting', this._boundOnUpgradeStarting));
    }

    unload(base) {
      return Promise.resolve()
      .then(() => this._upgradeManager.removeListener('upgrade-starting', this._boundOnUpgradeStarting))
      .then(() => this._removeUpgradeFromLogging())
      .then(() => {
        this._omgsDir = null;
        this._rawsDir = null;
        this._infosDir = null;
    ...
    loadOmg(id) {
      ...
      if (omg) {
        ...
        if (!this.isBaseAllowed(baseInfo)) {
        ...
        .then(() => this._upgradeManager.upgradeBase(romgFilepath));
      } else {
        return Promise.reject(new Error('omg/load-omg-does-not-exist'));
      }
    }

    isBaseAllowed(baseInfo) {
      if (!baseInfo) {
        return true;
      }
    ...
    _addUpgradeToLogging(messageCenter) {
      this._baseLoggingApi = new BaseLoggingApi(messageCenter);
      return Promise.resolve()
      .then(() => this._baseLoggingApi.addLogDirectory({dirpath: this._upgradeLogsDir}));
    }

    _removeUpgradeFromLogging() {
      return Promise.resolve()
      .then(() => {
        if (this._baseLoggingApi !== null) {
          return Promise.resolve()
          .then(() => this._baseLoggingApi.removeLogDirectory({dirpath: this._upgradeLogsDir}))
          .then(() => {
            this._baseLoggingApi = null;
          });
        }
    ...
    _addOmg(omg) {
      ...
      .then((header) => {
        ...
        .catch((err) => {
          throw new Error('Specified key(s) not found on device');
        });
      })
    ...
    _loadOmgUpgradeErrorFile(filename) {
      return UtilFs.readFile(filename, 'utf8')
      .then((errorFileContents) => {
        // file exits... save contents in the key-value store
        logger.info('UpgradeErrors: saving file contents');
        this._currentErrorData = errorFileContents;
      }, (err) => {
        if ('ENOENT' === err.code) {
          // no such file... save empty contents in the key-value store
          logger.info('UpgradeErrors: saving null');
          this._currentErrorData = null;
        }
      });
    }
