...
(() => {
  ...
  class ProxyManager {
    constructor(baseServer, moduleManager) {
      ...
      this._boundOnStrategyAdded = this._onStrategyAdded.bind(this);
      this._boundOnStrategyRemoved = this._onStrategyRemoved.bind(this);
      this._baseServer.on('base-server-strategy-used', this._boundOnStrategyAdded);
      this._baseServer.on('base-server-strategy-unused', this._boundOnStrategyRemoved);
    }

    _onStrategyAdded({name}) {
      strategies.push(name);
      this._proxies.forEach((proxy) => {
        if (proxy.auth) {
          this._baseServer.updateAuthStrategy(proxy.path, getAuthMiddleware());
        }
      });
    }

    _onStrategyRemoved({name}) {
    ...
    addProxy({workerId, path, host, isAuthenticated=true}={}) {
      ...
      .then((id) => {
        ...
        if (isAuthenticated) {
          proxyData.auth = getAuthMiddleware();
        } else {
          proxyData.auth = null;
        }
