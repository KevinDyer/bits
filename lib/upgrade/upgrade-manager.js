      ...
      .then(() => {
        ...
        return new Promise((resolve, reject) => {
          setTimeout(() => { // If the base upgrade works the node process will die before this fires
            reject(new Error('Unknown error with upgrade base timeout occured'));
          }, BASE_UPGRADE_TIMEOUT);
          return UtilChildProcess.createSpawnPromise('whereis', ['systemd-run'], options)
          .then((result) => {
            const exists = result.stdout.toString().split(/[ ]+/).length;
            if (exists > 1) {
              ...
              .then((result) => {
                if (0 === result.code) {
                  ...
                  const lastOut = result.stdout[result.stdout.length - 1].trim();
