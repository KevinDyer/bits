...
(() => {
  ...
  class UserManager extends EventEmitter {
    ...
    toggleDevelopmentScope({username} = {}) {
      ...
      .then((user) => {
        ...
        const scopes = (scopes.some(isDevScope) ? user.scopes.filter(!!isDevScope) : [ScopesManager.SCOPE_DEVELOPMENT].concat(user.scopes));
        return this.update(userId, {$set: {scopes}});
      });
    }
  ...
  function isDevScope({name} = {}) {
    return name === ScopesManager.SCOPE_DEVELOPMENT;
  }
