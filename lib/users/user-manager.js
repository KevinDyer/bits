...
(() => {
  ...
  const UserMessenger = require('./user-messenger');
  ...
  class UserManager extends EventEmitter {
    ...
    toggleDevelopmentScope({username} = {}) {
      return Promise.resolve()
      .then(() => this.getByUsername(username))
      .then((user) => {
        if (!user) return Promise.reject(new Error('user/invalid-username'));

        const scopes = (user.scopes.some(isDevScope) ? user.scopes.filter((s) => !isDevScope(s)) : [ScopesManager.SCOPE_DEVELOPMENT].concat(user.scopes));
        return this.update(user.id, {$set: {scopes}});
      });
    }
    ...
    _createPasswordHash(password, salt) {
      const shasum = UtilCrypto.createHash('sha256');
      shasum.update(password + salt, 'utf8');
      return shasum.digest('hex');
    }

    _createPasswordSalt() {
      return UtilCrypto.randomBytesSync(16).toString('hex');
    }
  ...
  function isDevScope({name} = {}) {
    return name === ScopesManager.SCOPE_DEVELOPMENT.name;
  }
