...
(() => {
  ...
  class UserMessenger extends CrudMessenger {
    static get REQUEST() {
      return {
        TOGGLE_DEV_SCOPE: `${UserApi.TAG} toggleDevScope`,
      };
    }
    ...
    constructor(manager) {
      ...
      this.addRequestListener(UserMessenger.REQUEST.TOGGLE_DEV_SCOPE, {scopes: this._writeScopes}, this._toggleDevScope.bind(this));
    }
    ...
    _toggleDevScope(metadata, request) {
      return Promise.resolve()
      .then(() => this._manager.toggleDevelopmentScope(request));
    }
